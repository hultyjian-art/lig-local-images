...
parentPort?.on('message', async (msg) => {
  const { id, abs, width, quality } = msg || {};
  try {
    const buf = await decodeThumb(abs, width, quality);
    if (!buf) {
      parentPort.postMessage({ id, ok: true, buf: null });
      return;
    }
    // 用 Transferable 转移字节，避免大 buffer 的结构化克隆开销
    const ab = buf.buffer.slice(buf.byteOffset, buf.byteOffset + buf.byteLength);
    parentPort.postMessage({ id, ok: true, buf: ab }, [ab]);
  } catch (e) {
